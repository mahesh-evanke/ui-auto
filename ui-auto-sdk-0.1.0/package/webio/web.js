...
/**
 * Extract interactable elements from the current page for AI feature generation.
 * Used by AI tab: no URL, no second browser — uses current document.
 */
(function () {
  if (!window.__WEBIO__) return;
  ...
  function xpathLit(s) {
    ...
    if (str.indexOf("'") === -1) return "'" + str + "'";
    return "concat(" + str.split('"').map(function (p) { return '"' + p + '"'; }).join(', \'"\', ') + ")";
  }
  function getXPath(el) {
    if (!el || el.nodeType !== 1) return "";
    var attr = function (n) { try { return el.getAttribute ? el.getAttribute(n) : null; } catch (e) { return null; } };
    var id = el.id ? String(el.id).trim() : "";
    if (id) return "//*[@id=" + xpathLit(id) + "]";
    var dt = attr("data-testid");
    if (dt && String(dt).trim()) return "//*[@data-testid=" + xpathLit(String(dt).trim()) + "]";
    var name = attr("name");
    if (name && String(name).trim()) return "//*[@name=" + xpathLit(String(name).trim()) + "]";
    var aria = attr("aria-label");
    if (aria && String(aria).trim()) return "//*[@aria-label=" + xpathLit(String(aria).trim()) + "]";
    var placeholder = attr("placeholder");
    var tag = (el.tagName || "").toLowerCase();
    if ((tag === "input" || tag === "textarea") && placeholder && String(placeholder).trim())
      return "//" + tag + "[@placeholder=" + xpathLit(String(placeholder).trim()) + "]";
    var text = String((el.innerText || el.textContent || "")).trim().replace(/\s+/g, " ");
    if ((tag === "button" || tag === "a") && text && text.length <= 80)
      return "//" + tag + "[normalize-space(.)=" + xpathLit(text) + "]";
    var parts = [], node = el;
    while (node && node.nodeType === 1 && node !== document.documentElement) {
  ...
  function getCss(el) {
    ...
    if (id) return "#" + id.replace(/([ !\"#$%&'()*+,./:;<=>?@[\\\]^`{|}~])/g, "\\$1");
    var dt = attr("data-testid");
    if (dt && String(dt).trim()) return '[data-testid="' + String(dt).trim().replace(/"/g, '\\"') + '"]';
    var name = attr("name");
    if (name && String(name).trim()) return '[name="' + String(name).trim().replace(/"/g, '\\"') + '"]';
    var aria = attr("aria-label");
    if (aria && String(aria).trim()) return '[aria-label="' + String(aria).trim().replace(/"/g, '\\"') + '"]';
    ...
    while (node && node.nodeType === 1 && node !== document.documentElement) {
      ...
      if (node.classList && node.classList.length) sel += "." + Array.from(node.classList).join(".");
      path.unshift(sel);
      node = node.parentElement;
    }
    return path.join(" > ");
  ...
  function pickSelector(el) {
    ...
    return { type: "css", value: getCss(el) || ("xpath:" + xp) };
  }
  function extractFromCurrentPage() {
    var sel = "input, textarea, button, a[href], select, [role=\"button\"], [role=\"link\"], [role=\"textbox\"], [onclick], [tabindex=\"0\"]";
    ...
    for (var i = 0; i < nodes.length; i++) {
      ...
      seen.add(xp);
      var s = pickSelector(el);
      var ln = getLogicalName(el);
      if (!ln) ln = "element_" + (i + 1);
      results.push({
        logicalName: ln,
        selectorType: s.type === "id" ? "css" : s.type,
        selectorValue: s.value,
        objectType: getObjType(el),
  ...
  ns.extractFromCurrentPage = extractFromCurrentPage;
})();
...
(() => {
    ...
    var persistedParentPath = panelSettings.parentPath || "";
    ...
    function updateListUI() {
        ...
        var aiContextEl = listUI.querySelector("#aiContextInput");
        var preservedAiContextValue = aiContextEl ? aiContextEl.value : null;
        var parentPathForTemplate = preservedPathValue !== null ? preservedPathValue : persistedParentPath;
        var exportPathSegment = (parentPathForTemplate || "").trim();
        ...
        listUI.innerHTML = `<style>[data-webio-panel] input,[data-webio-panel] select,[data-webio-panel] textarea{color:#000!important;background-color:#fff!important;border-color:#ccc!important}[data-webio-panel] button:not(#setScreenIdBtn):not(#recordingToggleBtn):not(#generateJsonBtn):not(#generateFeatureBtn):not(#generateApiFeatureBtn):not(#generateAiFeatureBtn):not(#webioHideBtn):not(.webio-tab){color:#000!important;background-color:#fff!important;border-color:#ccc!important}</style>
            ...
            <div class="webio-tabs" style="flex-shrink:0;display:flex;border-bottom:1px solid #e0e0e0;background:#f5f5f5;">
                ...
                <button type="button" id="webioTabAi" class="webio-tab" data-tab="ai" style="flex:1;padding:10px 12px;border:none;border-bottom:3px solid ${webioActiveTab === "ai" ? "#2563eb" : "transparent"};background:${webioActiveTab === "ai" ? "#fff" : "transparent"};font-weight:600;font-size:12px;cursor:pointer;color:#000;">AI</button>
            </div>
            ...
            <div id="webio-tab-ai" class="webio-tab-pane" style="display:${webioActiveTab === "ai" ? "flex" : "none"};flex:1;flex-direction:column;min-height:0;">
                <div class="webio-scroll webio-ai-scroll" style="flex:1;min-height:0;overflow-y:auto;overflow-x:hidden;padding:12px;-webkit-overflow-scrolling:touch;">
                    <div style="margin-bottom:12px;padding:12px;background:#f0fdf4;border:1px solid #16a34a;border-radius:8px;">
                        <div style="font-size:12px;font-weight:600;color:#000;margin-bottom:8px;">AI — Generate feature from current page</div>
                        <p style="margin:0 0 8px 0;font-size:11px;color:#333;">Uses the page you are on now. Enter a prompt to focus on specific elements (e.g. &quot;capture the Logout button&quot; or &quot;email, password and login button&quot;).</p>
                    </div>
                    <div style="margin-bottom:12px;">
                        <div style="font-size:11px;color:#000;margin-bottom:6px;font-weight:600;">Prompt / Context (optional)</div>
                        <textarea id="aiContextInput" placeholder="e.g. capture the Logout button, or: email, password and login button" rows="3" style="width:100%;padding:10px 12px;border:1px solid #ccc;border-radius:6px;font-size:12px;box-sizing:border-box;resize:vertical;">${escapeHtml(preservedAiContextValue !== null ? preservedAiContextValue : "")}</textarea>
                    </div>
                    <div id="aiGenerateStatus" style="margin-bottom:12px;font-size:12px;color:#666;min-height:20px;"></div>
                    <button id="generateAiFeatureBtn" type="button" style="width:100%;padding:10px 12px;border:1px solid #16a34a;border-radius:6px;background:#16a34a;color:#fff;cursor:pointer;font-size:12px;font-weight:600;">Generate Feature from Current Page</button>
                </div>
            </div>
            <div style="flex-shrink:0;display:flex;flex-direction:row;">
                <div class="webio-resize-handle" data-resize="height" style="flex:1;height:8px;min-height:8px;cursor:ns-resize;background:#e0e0e0;" title="Drag to resize height"></div>
                <div class="webio-resize-handle" data-resize="width" style="width:8px;min-width:8px;cursor:ew-resize;background:#e0e0e0;" title="Drag to resize width"></div>
                <div class="webio-resize-handle" data-resize="both" style="width:14px;height:14px;min-width:14px;min-height:14px;cursor:nwse-resize;background:#c0c0c0;flex-shrink:0;" title="Drag to resize both height and width"></div>
            </div>
        `;
        ...
        var newScrollEl = webioActiveTab === "webui"
            ...
            : webioActiveTab === "ai"
            ? listUI.querySelector("#webio-tab-ai .webio-scroll")
        ...
        function updateExportLocationDisplay() {
            var folder = (parentPathInput && parentPathInput.value || "").trim();
            var pathSegment = folder;
            if (exportLocationDisplay) {
                var suffix = pathSegment ? pathSegment + "/" : "";
                exportLocationDisplay.innerHTML = "pages/" + suffix + "<br>features/" + suffix;
            }
        }
        if (parentPathInput) {
            parentPathInput.oninput = parentPathInput.onchange = function () {
                persistedParentPath = (parentPathInput.value || "").trim();
                savePanelSettings({ lastScreenId: currentScreenId, parentPath: persistedParentPath });
        ...
        var generateAiFeatureBtn = listUI.querySelector("#generateAiFeatureBtn");
        if (generateAiFeatureBtn) {
            generateAiFeatureBtn.onclick = function () {
                var contextInput = listUI.querySelector("#aiContextInput");
                var statusEl = listUI.querySelector("#aiGenerateStatus");
                var context = (contextInput && contextInput.value) ? String(contextInput.value).trim() : "";
                if (typeof window.webioGenerateFromCurrentPage !== "function") {
                    if (statusEl) statusEl.textContent = "AI generation requires webio CLI (node webio/cli.js).";
                    alert("AI generation is only available when using the webio recorder. Run: node webio/cli.js");
                    return;
                }
                var rawData = (window.__WEBIO__ && typeof window.__WEBIO__.extractFromCurrentPage === "function")
                    ? window.__WEBIO__.extractFromCurrentPage()
                    : null;
                if (!rawData || !rawData.elements || rawData.elements.length === 0) {
                    if (statusEl) statusEl.textContent = "No interactable elements found on this page.";
                    alert("No interactable elements found on the current page. Ensure you are on a page with inputs, buttons, or links.");
                    return;
                }
                if (statusEl) statusEl.textContent = "Generating... (this may take a minute)";
                generateAiFeatureBtn.disabled = true;
                window.webioGenerateFromCurrentPage({ rawData: rawData, context: context, domain: "web" }).then(function (result) {
                    generateAiFeatureBtn.disabled = false;
                    if (statusEl) statusEl.textContent = "";
                    if (result && result.ok) {
                        alert("Feature generated successfully.\n\n" + (result.paths && result.paths.length ? result.paths.join("\n") : result.message));
                    } else {
                        if (statusEl) statusEl.textContent = result && result.message ? result.message : "Generation failed.";
                        alert("Generation failed: " + (result && result.message ? result.message : "unknown error"));
                    }
                }).catch(function (err) {
                    generateAiFeatureBtn.disabled = false;
        ...
        listUI.querySelectorAll(".webio-tab").forEach(function (tabBtn) {
            tabBtn.onclick = function () {
                ...
                if (tab && (tab === "webui" || tab === "api" || tab === "webui-api" || tab === "ai")) {
                    webioActiveTab = tab;
                    updateListUI();
                }
            };
    ...
    listUI.style.cssText = `
        ...
        width: ` + panelWidth + `px; min-width: 260px; max-width: 95vw; height: ` + panelHeight + `px; min-height: 200px; max-height: 90vh;
    ...
    var resizing = false, resizeMode = ""; var resizeStartY = 0, resizeStartHeight = 0, resizeStartX = 0, resizeStartWidth = 0;
    ...
    document.addEventListener("mousemove", function (e) {
        if (resizing) {
            if (resizeMode === "height") {
                ...
                var newW = Math.max(260, Math.min(window.innerWidth - 40, resizeStartWidth + dx));
                panelWidth = newW;
                listUI.style.width = newW + "px";
    ...
    function showBubbleView() {
        ...
        listUI.querySelector(".webio-bubble").addEventListener("click", function (e) {
            ...
            listUI.style.width = panelWidth + "px";
    ...
    function init() {
        ...
        listUI.addEventListener("mousedown", function (e) {
            ...
            if (resizeHandle && !panelMinimized) {
                e.preventDefault();
                resizing = true;
                resizeMode = resizeHandle.getAttribute("data-resize") || "height";
                resizeStartY = e.clientY;
                resizeStartHeight = panelHeight;
                resizeStartX = e.clientX;
                resizeStartWidth = listUI.offsetWidth;
            }
        });
