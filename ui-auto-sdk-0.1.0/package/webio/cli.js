 ...
 * When used as SDK dependency, uses consumer project root (process.cwd()) for e2e/config.
 */
const path = require("path");
const fs = require("fs");

// Use consumer project root for e2e paths (works for both main project and SDK installs).
const PROJECT_ROOT = path.resolve(process.env.INIT_CWD || process.env.UI_AUTO_PROJECT_ROOT || process.cwd());
process.env.UI_AUTO_PROJECT_ROOT = PROJECT_ROOT;

const { writeGeneratedFiles } = require("./generate-locators-and-features.js");
const { runGenerateFromUrl, runGenerateFromCurrentPage } = require("./generate-from-url.js");
const { getViewport } = require("./devicePresets.js");

function loadConfig() {
    const configPath = path.join(PROJECT_ROOT, "e2e", "config", "config.yaml");
...
async function runWithPuppeteer(scriptContent, startUrl, browserLabel, launchOpts) {
    ...
    const setupPage = async (page) => {
        ...
        try {
            ...
            await page.exposeFunction("webioGenerateFromUrl", async (opts) => {
                return runGenerateFromUrl(opts || {});
            });
            await page.exposeFunction("webioGenerateFromCurrentPage", async (opts) => {
                return runGenerateFromCurrentPage(opts || {});
            });
