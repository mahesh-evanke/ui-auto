...
Given('User navigates to {string} URL', async (url: string) => {
    // Use wait: 'interactive' so navigation returns when DOM is ready, avoiding long full-load waits on heavy pages
    await browser.url(url, { wait: 'interactive' } as any);
    await browser.pause(1000);
    // Wait for URL to reflect navigation so next step (User is on screen) can assert title
    try {
        const hostname = new URL(url).hostname;
        await browser.waitUntil(async () => (await browser.getUrl()).includes(hostname), { timeout: 15000 });
        await browser.pause(2000);
    } catch (e) { /* ignore */ }
