...
## DATABASE Statements (Setup & Verification)

**Source:** `e2e/stepdefinitions/db/database.steps.ts`  
**Config:** Set `db` in `e2e/config/config.yaml` (type, host, port, user, password, database) or use `Given the database connection:` with a credentials table.

### Given (database)

| Step | Purpose |
|------|---------|
| `Given the database connection:` | Provide DB credentials in a table. Use **pgsql** (columns: type \| host \| port \| user \| password \| database) or **sqlite** (type \| path). If omitted, uses `db` from config.yaml. |

### Then (database – SQL verification)

| Step | Purpose |
|------|---------|
| `Then the database table "{table}" should have at least N row(s)` | Asserts table has ≥ N rows. |
| `Then the database table "{table}" should have at most N row(s)` | Asserts table has ≤ N rows. |
| `Then the database table "{table}" should have exactly N row(s)` | Asserts table has exactly N rows. |
| `Then the database table "{table}" should have row where column "{col}" equals "{value}"` | Asserts at least one row has column = value. |
| `Then the database table "{table}" should contain value "{value}" in column "{col}"` | Same as above (alternative wording). |
| `Then the database table "{table}" should have no rows where column "{col}" equals "{value}"` | Asserts zero rows match column = value. |
| `When User verifies info from the sql query "{sql}"` (with table) | Runs SQL and compares result to the DataTable. |
...
Scenario: Verify users table
  ...
  Then the database table "users" should have row where column "email" equals "user@example.com"
```
